...
async function main() {
  ...
  // call DCA's distribute
  console.log("let's activate the swap and return the output to the investors");
  const amountToDistribute = hre.ethers.utils.parseEther("1000");
  ret = await dcaContract.connect(signer).distributeTokens(amountToDistribute);
